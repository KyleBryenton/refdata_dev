...
function file = namefile(edir,tag)
  file = sprintf("%s/%s.nwout",edir,tag);
endfunction
...
function [c6,c8,c10,rc] = readcij(file,n)
  ...
  line = fgetl(fid);
  do 
    if(regexp(line,"XDM: dispersion coefficients"));
      line = fgetl(fid);
      line = fgetl(fid);
      line = fgetl(fid);
      do
...
function [e edisp] = readenergy(file)
  [stat,out] = system(sprintf("grep 'Total DFT energy' %s | awk '{print $5}'\n",file));
  if (length(out) == 0)
    e = 0;
  else
    e = str2num(out);
  endif
  [stat,out] = system(sprintf("grep 'Dispersion correction' %s | awk '{print $4}'\n",file));
  if (length(out) != 0)
    edisp = str2num(out);
