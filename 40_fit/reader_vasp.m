...
function file = namefile(edir,tag)
  file = sprintf("%s/%s/%s.outcritic",edir,tag,tag);
endfunction
...
function [e edisp] = readenergy(file)
  reg = regexp(file,"/[!/]*");
  file2 = strcat(file(1:reg(end)),"OSZICAR");
  [stat,out] = system(sprintf("grep 'F' %s | awk '{print $3}'\n",file2));
  if (length(out) == 0)
    e = 0;
  else
    e = str2num(out) / 27.211385; # to hartree
  endif
