...
## function readenergy: read energy from the energy.dat in the root diretory
## Each line in the energy.dat file must contain:
##     name   energy
## where name is the name of the molecule and energy is the energy in Hartree.
function [e edisp] = readenergy(file)
  posb = findstr(file,"/");
  name = file(posb(end)+1:end);
  name = strrep(name,".scf.out","");
  file = strcat(file(1:posb(end-1)),"energy.dat");
  if (!exist(file,"file"))
    error(sprintf("File %s not found!",file))
  endif

  [s out] = system(sprintf("grep '^%s ' %s | tail -n 1 | awk '{print $NF}'",name,file));
  if (length(out) == 0)
    e = 0;
  else
    e = str2num(out);
